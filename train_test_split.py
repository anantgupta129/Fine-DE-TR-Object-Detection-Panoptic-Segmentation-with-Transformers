            ...
            if count % test_size != 0:
                ...
                while True:
                    if (
                        ...
                        and data["annotations"][k]["image_id"] == im_id
                    ):

                        data["annotations"][k]["image_id"] = train_im_id
                        data["annotations"][k]["id"] = train_anno_id

                        train["annotations"].append(data["annotations"][k])
                        k += 1
                        train_anno_id += 1
                    else:
                        break
                ...
                while True:
                    if (
                        ...
                        test["annotations"].append(data["annotations"][k])
                        k += 1
                        test_anno_id += 1
                    else:
                        break
    ...
    for cat in train["annotations"]:
        ...
        if type(category_id) == str:
            if "miscellaneous" in category_id:
                ...
                cat["category_id"] = (
                    cat_count
                    + coco_cat_to_id[coco_id_to_cat[int(category_id.split(":")[-1])]]
                )
